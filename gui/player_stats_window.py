import time
from tkinter import Button, Checkbutton, Entry, Frame, IntVar, Label, Spinbox, Toplevel
...
class PlayerStatsWindow(Toplevel):
    ...
    def __init__(self, master, player:Player, nations):
        ...
        player_idx_label = Label(self, text="Player ID")
        ...
        player_name_label = Label(self, text="Player Name")
        ...
        player_shirt_name_label = Label(self, text="Player Shirt Name")
        ...
        player_callname_label = Label(self, text=player.callname.name)
        ...
        player_nationality_label = Label(self, text=player.nation.name)
        ...
        player_age_label = Label(self, text=player.basic_settings.age.name)
        player_age_label.grid(row=5, column=0, sticky="e")
        self.player_age_int_var = IntVar(self, player.basic_settings.age())
        player_age_spinbox = Spinbox(self, from_= 15, to = 46, textvariable=self.player_age_int_var, width = 5)
        player_age_spinbox.grid(row=5, column=1, sticky="w")

        player_stronger_foot_label = Label(self, text=player.basic_settings.stronger_foot.name)
        player_stronger_foot_label.grid(row=6, column=0, sticky="e")
        self.player_stronger_foot_combobox = Combobox(self, state="readonly", value=FOOT_FAV_SIDE[:2], width=20)
        ...
        player_injury_label = Label(self, text=player.basic_settings.injury.name)
        player_injury_label.grid(row=7, column=0, sticky="e")
        self.player_injury_combobox = Combobox(self, state="readonly", value=INJURY_VALUES, width=20)
        self.player_injury_combobox.set(player.basic_settings.injury())
        self.player_injury_combobox.grid(row=7, column=1, sticky="w")

        player_style_of_dribble_label = Label(self, text=player.basic_settings.style_of_dribble.name)
        ...
        player_free_kick_type_label = Label(self, text=player.basic_settings.free_kick_type.name)
        ...
        player_penalty_kick_label = Label(self, text=player.basic_settings.penalty_kick.name)
        ...
        player_drop_kick_style_label = Label(self, text=player.basic_settings.drop_kick_style.name)
        ...
        player_goal_celebration_1_label = Label(self, text=player.basic_settings.goal_celebration_1.name)
        player_goal_celebration_1_label.grid(row=12, column=0, sticky="e")
        self.player_goal_celebration_1_combobox = Combobox(self, state="readonly", value=["No"] + list(range(1,128)), width=20)
        ...
        player_goal_celebration_2_label = Label(self, text=player.basic_settings.goal_celebration_2.name)
        player_goal_celebration_2_label.grid(row=13, column=0, sticky="e")
        self.player_goal_celebration_2_combobox = Combobox(self, state="readonly", value=["No"] + list(range(1,128)), width=20)
        ...
        player_growth_type_label = Label(self, text=player.basic_settings.growth_type.name)
        player_growth_type_label.grid(row=14, column=0, sticky="e")
        self.player_growth_type_combobox = Combobox(self, state="readonly", value=GROWTH_TYPE_NAMES, width=20)
        self.player_growth_type_combobox.grid(row=14, column=1, sticky="w")

        player_unknown_growth_type_label = Label(self, text="%s value" % player.basic_settings.growth_type.name)
        player_unknown_growth_type_label.grid(row=15, column=0, sticky="e")
        player_growth_type_value_var = IntVar(self, player.basic_settings.growth_type())
        player_unknown_growth_type_entry = Entry(self, width=6, textvariable=player_growth_type_value_var)
        ...
        player_height_label = Label(self, text=player.appearance.height.name)
        ...
        player_weight_label = Label(self, text=player.appearance.weight.name)
        ...
        player_face_label = Label(self, text=player.appearance.face.name)
        player_face_label.grid(row=19, column=0, sticky="e")
        self.player_face_combobox = Combobox(self, state="readonly", value=FACE_TYPE, width=20)
        self.player_face_combobox.set(player.appearance.face())
        self.player_face_combobox.grid(row=19, column=1, sticky="w")

        player_skin_colour_label = Label(self, text=player.appearance.skin_colour.name)
        player_skin_colour_label.grid(row=20, column=0, sticky="e")
        self.player_skin_colour_combobox = Combobox(self, state="readonly", value=list(range(1,7)), width=20)
        ...
        player_head_height_label = Label(self, text=player.appearance.head_height.name)
        player_head_height_label.grid(row=21, column=0, sticky="e")
        self.player_head_height_int_var = IntVar(self, player.appearance.head_height())
        player_head_height_spinbox = Spinbox(self, from_= -7, to = 7, textvariable=self.player_head_height_int_var, width = 5)
        player_head_height_spinbox.grid(row=21, column=1, sticky="w")

        player_head_width_label = Label(self, text=player.appearance.head_width.name)
        player_head_width_label.grid(row=22, column=0, sticky="e")
        self.player_head_width_int_var = IntVar(self, player.appearance.head_width())
        player_head_width_spinbox = Spinbox(self, from_= -7, to = 7, textvariable=self.player_head_width_int_var, width = 5)
        player_head_width_spinbox.grid(row=22, column=1, sticky="w")

        player_face_idx_label = Label(self, text=player.appearance.face_idx.name)
        player_face_idx_label.grid(row=23, column=0, sticky="e")
        self.player_face_idx_int_var = IntVar(self, player.appearance.face_idx())
        self.player_face_idx_spinbox = Spinbox(self, from_= MIN_FACE_IDX, to = MAX_FACE_IDX, textvariable=self.player_face_idx_int_var, width = 5)
        self.player_face_idx_spinbox.grid(row=23, column=1, sticky="w")

        player_hair_idx_label = Label(self, text=player.appearance.hair.name)
        player_hair_idx_label.grid(row=24, column=0, sticky="e")
        self.player_hair_idx_int_var = IntVar(self, player.appearance.hair())
        self.player_hair_idx_spinbox = Spinbox(self, from_= MIN_HAIR_IDX, to = MAX_HAIR_IDX, textvariable=self.player_hair_idx_int_var, width = 5)
        ...
        player_special_hairstyles_2_label = Label(self, text=player.appearance.special_hairstyles_2.name)
        player_special_hairstyles_2_label.grid(row=25, column=0, sticky="e")
        self.player_special_hairstyles_2_var = IntVar(self, NO_YES.index(player.appearance.special_hairstyles_2()))
        ...
        apply_button = Button(self, text="Apply", command=lambda: self.update_player_data(player))
        apply_button.grid(row=29, column=0, sticky="e")
        cancel_button = Button(self, text="Cancel", command=lambda :self.stop_window())
        cancel_button.grid(row=29, column=1, sticky="w")
        psd_button = Button(self, text = "PSD Paste", command= lambda : self.psd.paste_psd(self, self.selection_get(selection = "CLIPBOARD"), nations))
        psd_button.grid(row=30, column=0, sticky="e")
        psd_button = Button(self, text = "PSD Copy", command= lambda : self.to_clipboard())
        ...
        for ability in player.abilities_1_8:
            ...
            e = Entry(self, width=3, textvariable=self.abilities_1_8_entries[ability.name])
            e.grid(row=column_2_row_counter, column=3, sticky="e")
        ...
        player_favored_side_label = Label(self, text=player.position.favored_side.name)
        player_favored_side_label.grid(row=5, column=6, sticky="e")
        self.player_favored_side_combobox = Combobox(self, state="readonly", value=FOOT_FAV_SIDE, width=20)
        ...
        player_registered_position_label = Label(self, text=player.position.registered_position.name)
        ...
        """
        # test for face and hair preview
        test_frame = Frame(self)
        test_frame.grid(row=column_6_row_counter, column=6, columnspan=2)
        Label(test_frame, bg="red", width=15, height=7).pack()
        Label(test_frame, bg="blue", width=20, height=7).pack()
        """
        self.lift()
        self.grab_set()
